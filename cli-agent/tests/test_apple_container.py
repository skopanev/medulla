...
def apple():
    ...
    module.shared.env_file_for_run = None
    module.shared.shadow_paths_for_run = []
    return module
...
def test_build_run_command_matches_medulla_mount_and_sandbox_contract(
    ...
    apple.shared.env_file_for_run = "/tmp/merged.env"
    apple.shared.shadow_paths_for_run = ["secrets"]
...
def test_interrupted_build_signals_only_its_client_group(
    ...
    monkeypatch.setattr(
        apple, "signal_process_group", lambda target, signum: forwarded.append((target, signum)))
