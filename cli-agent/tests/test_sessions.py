"""A named session lets a later node reach the SAME conversation.

Without it every agent node is a fresh conversation, so a workflow that hands work
to an agent, acts on the result outside the container, and then needs the agent
again makes it re-derive everything the first turn knew. A rejected push or a rebase
conflict is exactly when that context is worth most.

Two halves are tested here: the engine's plumbing (record the id, hand it back to the
next node with the same name) and each adapter's own command for continuing — the
four CLIs disagree about the word, the shape, and even whether it is a flag.
"""
import json
